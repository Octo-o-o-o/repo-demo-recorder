...
**MUST ASK FIRST — 数据来源 / 录制环境（三选一）**。这一项决定后续整条流水线（auth、baseUrl、seed、storageState、安全边界）的走向，不允许默认推断，必须显式问用户：

- `mock`（默认推荐）：本地 dev 环境，演示数据由 seed 脚本 / fixture / API intercept 提供。**没有真实用户隐私，可以反复重录、可以入镜**。客户演示、培训 SOP、CI 录制都用这种。scaffold 跑 `--data-mode mock`。
- `staging`：staging / pre-prod / 测试环境。已有专门的演示账号 / 演示租户，所有数据是为 demo 准备的。可以演示登录后业务，但需要 `auth.storageState` 或 dev-login 拿到 session。scaffold 跑 `--data-mode staging`。
- `production`：真实生产数据。**默认拒绝**——必须用户明确确认这是合规场景（如客户自己看自己的工作台、用户授权的 case study），并显式传 `--data-mode production --allow-production`。scaffold 会强制写入合规警告到 RECORDING_GUIDE 和 scenario，禁止 UI 写入、禁止登录他人账号、禁止把别的客户的数据拍进画面。如果用户语焉不详，**问清楚再录**。

询问数据来源时附带说明：「mock 安全可发版；staging 适合工程验收，要预先准备好演示账号；production 风险高，需要客户书面授权」。

**MUST ASK NEXT — 真正想演示什么业务**。光看 landing 页面只是公司官网而不是产品；要让画面进入登录后的核心业务（dashboard、表单、列表、详情、报表）。如果用户只描述了产品名没说"演示哪一段"，至少问清"演示哪条核心流程 + 关键的 1-3 个动作"，然后据此扩 `scenario.flows[].steps`，不要靠默认骨架糊弄过去（默认骨架只是 landing 浏览，没有任何产品本体）。

其余选项（默认值能用就用默认，不必每次都问）：
...
- **数据写入策略**（不同于上面"数据来源"，这一项决定 UI 是否真的写入）：默认 readonly。选项：只读演示、真实 UI 写入（DB 写入前必须备份）、API seed、Prisma seed、录后清理。production 模式下默认强制 readonly。
...
- 软链 `node_modules` / `.env` / `.env.local` / `.env.development*` / `.env.test*` 到 worktree，避免重新安装依赖、重新填常规 dev/test 环境变量。`.env.production*` **不默认软链或 carry**；只有生产录制已被用户明确授权时，才用 `--link .env.production.local` 显式追加。其它构建缓存（`.next` / `.vite` / `dist`）**不**默认软链，因为主工作树并行启动会互相覆盖；如确认安全，用 `--link <relPath>` 显式追加。
...
新建的演示账号常会触发首登 onboarding 弹窗、隐私同意 banner、新人引导浮层。这些会叠在每个 dashboard 页面上方，让录出来的视频看起来像"runner 只录到了 onboarding 弹窗"。

**优先用 `scenario.preflight.steps` 在正式录制开始之前关掉它们**，而不是把 dismiss modal 的步骤写进 `flow.steps`（那样会把弹窗录进视频）。preflight 跟正式录制共用同一个浏览器上下文（cookie / localStorage / session 都生效），但不会进入视频、captions、report 时间线。

支持的 step 类型：`goto / click / fill / wait / fetch`。

最干净的写法是 `fetch` 直接 PATCH 项目的 profile API：
...
如果 modal 是纯前端控制（没有 API），用 click 走完即可：
...
如果 gate 状态存在 `localStorage` 里，优先把对应的 entry 写进 `auth.storageState` 的 `origins[].localStorage` 数组——比每次录制都点一遍要快。
...
- `scripts/prepare-recording-worktree.mjs`：在目标项目里 `git worktree add` 一个隔离录制环境，自动软链 `node_modules` / 非生产 `.env*`，可选 carry 未提交改动（默认跳过 `.env.production*`）；输出 worktreePath 元数据供 cleanup 使用。
